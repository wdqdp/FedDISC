...
class CMUMOSIDataset(Dataset):
    ...
    def __init__(self, label_path, audio_root, text_root, video_root):
        ...
        name2audio, adim = read_data(label_path, audio_root)  # name2audio为名称为key，特征为value的字典，adim为特征维度
        ...
        for ii, vid in enumerate(sorted(self.videoIDs)):
            uids = self.videoIDs[vid]  # videoIDs是字典，key为对话的id，值为对话里面每个utter的id
            labels = self.videoLabels[vid]
            speakers = self.videoSpeakers[vid]

            self.max_len = max(self.max_len, len(uids)) # 应该是所有对话中utter最多的数量
            speakermap = {'': 0}
            ...
            for ii, uid in enumerate(uids):
                self.videoAudioHost[vid].append(name2audio[uid])  # videoAudioHost的key为对话id，值为一个list，里面是所有切片的特征

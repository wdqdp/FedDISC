...
class MMDataset(Dataset):
    def __init__(self, args, client_idx, mode='train'):
        self.mode = mode
        self.args = args
    ...
    def __init_mosi(self):
        with open(self.args['featurePath'], 'rb') as f:
            data = pickle.load(f)
        if self.mode == 'train':
            client_len = data[self.mode]['vision'].shape[0] // self.args.client_num

            if 'use_bert' in self.args and self.args['use_bert']:
                self.text = data[self.mode]['text_bert'].astype(np.float32)[self.client_idx*client_len : (self.client_idx+1)*client_len]
            else:
                self.text = data[self.mode]['text'].astype(np.float32)[self.client_idx*client_len : (self.client_idx+1)*client_len]
            self.vision = data[self.mode]['vision'].astype(np.float32)[self.client_idx*client_len : (self.client_idx+1)*client_len]
            self.audio = data[self.mode]['audio'].astype(np.float32)[self.client_idx*client_len : (self.client_idx+1)*client_len]
            self.raw_text = data[self.mode]['raw_text'][self.client_idx*client_len : (self.client_idx+1)*client_len]
            self.ids = data[self.mode]['id'][self.client_idx*client_len : (self.client_idx+1)*client_len]
            self.labels = {
            'M': np.array(data[self.mode]['classification_labels']).astype(np.float32)[self.client_idx*client_len : (self.client_idx+1)*client_len]
            }
        else:
            if 'use_bert' in self.args and self.args['use_bert']:
...
def MMDataLoader(args, num_workers):
    ...
    for client_idx in range(args.client_num):
        ...
        datasets = {
            'train': MMDataset(args, client_idx, mode='train'),
            'valid': MMDataset(args, client_idx, mode='valid'),
            'test': MMDataset(args, client_idx, mode='test')
        }
        ...
        dataLoader_all[client_idx] = dataLoader
        a = 1

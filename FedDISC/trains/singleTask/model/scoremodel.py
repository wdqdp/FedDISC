...
def loss_fn(model, x, marginal_prob_std, condition=None, eps=1e-5):
  ...
  if condition is not None:
    ...
    perturbed_x = perturbed_x + condition
    score = model(perturbed_x, random_t)
  else:
    score = model(perturbed_x, random_t)
  loss = torch.mean(torch.sum((score * std[:, None, None] + z)**2, dim=(1,2)))
  # loss = torch.mean((score * std[:, None, None] + z)**2)
    ...
    with torch.no_grad():
        for time_step in time_steps:
            ...
            if condition is not None:
                ...
                mean_x = x + (g ** 2)[:, None, None] * score_model(x+perturbed_condition, batch_time_step) * step_size
            else:

...
class MetricsTop():
    def __init__(self, train_mode):
        if train_mode == "regression":
            ...
            self.metrics_dict = {
                ...
                'IEMOCAP4': self.__eval_iemocap4_classification,
                'IEMOCAP6': self.__eval_iemocap6_classification,
            }
    ...
    def __eval_iemocap4_classification(self, y_pred, y_true):
        """
        {
            "Negative": 0,
            "Neutral": 1,
        ...
        Mult_acc_4 = accuracy_score(y_pred_4, y_true)
        F1_score_4 = f1_score(y_true, y_pred_4, average='weighted')

        # 新增：计算每个类别的准确率（实际为召回率）
        precision, recall, f1, support = precision_recall_fscore_support(
            y_true, y_pred_4, average=None, labels=[0, 1, 2, 3]
        )
        
        # 构建类别准确率字典
        class_accuracy = {
            "recall_0_acc": round(recall[0], 4),
            "recall_1_acc": round(recall[1], 4),
            "recall_2_acc": round(recall[2], 4),
            "recall_3_acc": round(recall[3], 4),
            "precision_0_acc": round(precision[0], 4),
            "precision_1_acc": round(precision[1], 4),
            "precision_2_acc": round(precision[2], 4),
            "precision_3_acc": round(precision[3], 4)
        }

        eval_results = {
            "Acc_4": round(Mult_acc_4, 4),
            "F1_score_4": round(F1_score_4, 4),
            **class_accuracy  # 合并类别准确率
        }
        return eval_results
    
    def __eval_iemocap6_classification(self, y_pred, y_true):
        """
        {
            "Negative": 0,
            "Neutral": 1,
        ...
        eval_results = {
            "Acc_6": round(Mult_acc_6, 4),
            "F1_score_6": round(F1_score_6, 4)
        }
        return eval_results
